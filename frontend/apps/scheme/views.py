...
def get_scheme(sequence_id=1):
    ...
    for dl_association in ListSequenceAssociations.objects.filter(sequence_id=ds.id):
        dl = DataList.objects.get(id=dl_association.list_id)
        scheme[ds][dl] = []
        for di_association in ItemListAssociations.objects.filter(list_id=dl.id):
